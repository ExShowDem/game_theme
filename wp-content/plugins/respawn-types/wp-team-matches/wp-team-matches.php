<?php
/**
 * Plugin Name: WP-TeamMatches
 * Author URI: http://www.codeispoetry.ru/
 * Plugin URI: https://github.com/pronebird/wp-team-matches
 * Description: Content manager for e-sports teams and communities
 * Author: Andrej Mihajlov
 * Version: 2.0.1
 *
 * Tags: esport, teamwar, team, team, game, gaming, sport, match
 **/

/*
    WP-TeamMatches plugin
...
global $wpTeamMatches;

define('WP_CLANWARS_VERSION', '2.0.1');

define('WP_CLANWARS_TEXTDOMAIN', 'wp-team-matches');
define('WP_CLANWARS_COUNTRIES_TEXTDOMAIN', 'wp-team-matches-countries');

define('WP_CLANWARS_CATEGORY', '_wp_team-matches_category');
define('WP_CLANWARS_DEFAULTCSS', '_wp_team-matches_defaultcss');
define('WP_CLANWARS_DBVERSION', '_wp_team-matches_dbversion');

define('WP_CLANWARS_URL', WP_PLUGIN_URL . '/' . dirname(plugin_basename(__FILE__)));

// this folder is created in wp-content/
define('WP_CLANWARS_EXPORTDIR', 'wp-team-matches');
...
require_once(dirname(__FILE__) . '/wp-team-matches-slider-widget.php');
require_once(dirname(__FILE__) . '/wp-team-matches-list-widget.php');
require_once (ABSPATH . 'wp-admin/includes/class-pclzip.php');

// namespace import
use \WP_TeamMatches\Flash;
use \WP_TeamMatches\Utils;
use \WP_TeamMatches\View;
use \WP_TeamMatches\API as CloudAPI;

class WP_TeamMatches {
    ...
    function on_init()
    {
        ...
        add_action('admin_post_wp-team-matches-deleteteams', array($this, 'on_admin_post_deleteteams'));
        add_action('admin_post_wp-team-matches-sethometeam', array($this, 'on_admin_post_sethometeam'));
        add_action('admin_post_wp-team-matches-gamesop', array($this, 'on_admin_post_gamesop'));
        add_action('admin_post_wp-team-matches-deletemaps', array($this, 'on_admin_post_deletemaps'));
        add_action('admin_post_wp-team-matches-delete-match', array($this, 'on_admin_post_delete_match'));

        add_action('admin_post_wp-team-matches-settings', array($this, 'on_admin_post_settings'));
        add_action('admin_post_wp-team-matches-acl', array($this, 'on_admin_post_acl'));
        add_action('admin_post_wp-team-matches-deleteacl', array($this, 'on_admin_post_deleteacl'));
        add_action('admin_post_wp-team-matches-import', array($this, 'on_admin_post_import'));
        add_action('admin_post_wp-team-matches-publish', array($this, 'on_admin_post_publish'));
        add_action('admin_post_wp-team-matches-login', array($this, 'on_admin_post_login'));
        add_action('admin_post_wp-team-matches-logout', array($this, 'on_admin_post_logout'));
        add_action('admin_post_wp-team-matches-update-cloud-account', array($this, 'on_admin_post_update_cloud_account'));
        add_action('wp_ajax_wp-team-matches-game-vote', array($this, 'on_ajax_game_vote'));

        add_action('admin_post_wp-team-matches-setupteam', array($this, 'on_admin_post_setup_team'));
        add_action('admin_post_wp-team-matches-setupgames', array($this, 'on_admin_post_setup_games'));

        add_action('wp_ajax_get_maps', array($this, 'on_ajax_get_maps'));
        add_shortcode('wp-team-matches', array($this, 'on_shortcode'));
...
echo <<<EOT
<style type="text/css">
#toplevel_page_wp-team-matches-matches .wp-menu-image {
...
EOT;
    ...
    function on_admin_menu()
    {
        ...
        $acl_table = array(
            ...
            'manage_players' => 'manage_options',
            'manage_games' => 'manage_options',
        );

        $routes = array(
            'manage_matches' => 'wp-team-matches-matches',
            'manage_teams' => 'wp-team-matches-teams',
            'manage_players' => 'wp-team-matches-players',
            'manage_games' => 'wp-team-matches-games'
        );
        ...
        foreach( $acl_table as $custom_cap => $wordpress_cap ) {
            $has_permissions = \WP_TeamMatches\ACL::user_can( $custom_cap );
            if(!$has_permissions) {
                continue;
            }
        ...
        $top = add_menu_page(
            __('TeamMatches', WP_CLANWARS_TEXTDOMAIN),
            __('TeamMatches', WP_CLANWARS_TEXTDOMAIN),
            $user_role,
            $top_level_slug,
            null,
        ...
        $this->page_hooks['teams'] = add_submenu_page(
            ...
            $routes['manage_teams'],
            $this->onboarding_or_page( 'on_teams_redirect' )
        );

        $this->page_hooks['players'] = add_submenu_page(
            $top_level_slug,
            __('Players', WP_CLANWARS_TEXTDOMAIN),
            __('Players', WP_CLANWARS_TEXTDOMAIN),
            $acl_table['manage_players'],
            $routes['manage_players'],
            $this->onboarding_or_page( 'on_players_redirect' )
        );
        ...
        $this->page_hooks['import'] = add_submenu_page(
            $top_level_slug,
            __('Team Matches Cloud', WP_CLANWARS_TEXTDOMAIN),
            __('Team Matches Cloud', WP_CLANWARS_TEXTDOMAIN),
            'manage_options',
            'wp-team-matches-cloud',
        ...
        if(!$this->should_onboard_user()) {
            ...
            add_action('load-' . $this->page_hooks['players'], array($this, 'on_load_manage_players'));
    ...
    {
        wp_register_script('wp-team-matches-matches', WP_CLANWARS_URL . '/js/matches.js', array( 'jquery', 'select2' ), WP_CLANWARS_VERSION);
        wp_register_script('wp-team-matches-gallery', WP_CLANWARS_URL . '/js/gallery.js', array('jquery', 'jquery-ui-sortable', 'media-upload'), WP_CLANWARS_VERSION);

        wp_register_script('wp-team-matches-admin', WP_CLANWARS_URL . '/js/admin.js', array('jquery', 'select2'), WP_CLANWARS_VERSION);
        wp_localize_script('wp-team-matches-admin',
            'wpCWAdminL10n',
            array(
        ...
        wp_register_script('wp-team-matches-game-browser', WP_CLANWARS_URL . '/js/game-browser.js', array('jquery'), WP_CLANWARS_VERSION);
        wp_register_script('wp-team-matches-login', WP_CLANWARS_URL . '/js/login.js', array('jquery', 'jquery-tipsy'), WP_CLANWARS_VERSION);

        wp_register_style('wp-team-matches-admin', WP_CLANWARS_URL . '/css/admin.css', array( 'select2', 'jquery-tipsy', 'font-awesome', 'wp-admin' ), WP_CLANWARS_VERSION);
        wp_register_style('wp-team-matches-flags', WP_CLANWARS_URL . '/css/flags.css', array(), '1.01');
        ...
        wp_register_script('wp-team-matches-public', WP_CLANWARS_URL . '/js/public.js', array('jquery-tipsy'), WP_CLANWARS_VERSION);

        wp_register_style('wp-team-matches-sitecss', WP_CLANWARS_URL . '/css/site.css', array(), WP_CLANWARS_VERSION);
        wp_register_style('wp-team-matches-widgetcss', WP_CLANWARS_URL . '/css/widget.css', array(), WP_CLANWARS_VERSION);

        $callbackURL = admin_url( 'admin-post.php?action=wp-team-matches-login' );
        $facebook_login_url = CloudAPI::get_login_url('facebook', $callbackURL);
        $steam_login_url = CloudAPI::get_login_url('steam', $callbackURL);
        wp_localize_script('wp-team-matches-login',
            'wpTeamMatchesLoginSettings',
            compact('facebook_login_url', 'steam_login_url')
        );
    }

    function add_game_browser_script() {
        wp_enqueue_script('wp-team-matches-game-browser');
            wp_localize_script('wp-team-matches-game-browser',
            'wpTeamMatchesGameBrowserSettings',
            array(
                'vote_action' => 'wp-team-matches-game-vote',
                'vote_nonce' => wp_create_nonce('wp-team-matches-game-vote')
            )
        );
    }
    ...
    function should_onboard_user() {
        ...
        if($flag === null) {
            $has_hometeam = !empty( \WP_TeamMatches\Teams::get_hometeam() );
            $games = \WP_TeamMatches\Games::get_game(array(), true);
    ...
    function onboarding_setup_team_page($page_submit) {
        ...
        $view->add_helper('html_country_select_helper', array('\\WP_TeamMatches\\Utils', 'html_country_select_helper'));
    ...
    function onboarding_setup_games_page($page_submit) {
        ...
        $installed_games = \WP_TeamMatches\Games::get_game('');
        ...
        $active_tab = '';
        $install_action = 'wp-team-matches-import';
    ...
    function on_admin_post_setup_team() {
        ...
        check_admin_referer('wp-team-matches-setupteam');
        ...
        if( !empty( $data['title'] ) && !empty( $data['country'] ) ) {
            \WP_TeamMatches\Teams::add_team( $data );
        }
        else {
    ...
    function on_admin_post_setup_games() {
        ...
        check_admin_referer('wp-team-matches-setupgames');
        ...
        if( $import === 'upload') {
            ...
            if( !empty ( $new_game_name ) ) {
                $new_game_id = \WP_TeamMatches\Games::add_game(array(
                    'title' => $new_game_name,
                ...
                if($new_game_id === false) {
                    ...
                    $redirect_url = admin_url('admin.php?page=wp-team-matches-games&act=maps&game_id=' . $new_game_id);
                }
            }
            else {
    ...
    function on_template_redirect() {
        wp_enqueue_script('wp-team-matches-public');
        wp_enqueue_style('jquery-tipsy');
        wp_enqueue_style('wp-team-matches-flags');
    }

    function on_wp_footer() {
        if(get_option(WP_CLANWARS_DEFAULTCSS)) {
            wp_enqueue_style('wp-team-matches-sitecss');
            wp_enqueue_style('wp-team-matches-widgetcss');
        }
    }

    function on_load_any() {
        wp_enqueue_style('wp-team-matches-admin');
        wp_enqueue_style('wp-team-matches-flags');
        wp_enqueue_script('wp-team-matches-admin');

        add_filter( 'admin_body_class', array($this, 'on_admin_body_class') );

        // override page title during onboarding
        if($this->should_onboard_user()) {
            $this->set_page_title( __( 'Get started with WP-TeamMatches', WP_CLANWARS_TEXTDOMAIN ) );
        }
    }
    ...
    function on_admin_body_class($classes) {
        return trim("$classes wp-team-matches");
    }

    function on_widgets_init()
    {
        register_widget('WP_TeamMatches_Slider_Widget');
        register_widget('WP_TeamMatches_List_Widget');
    }
    ...
    {
        if( !\WP_TeamMatches\ACL::user_can('manage_teams') ) {
            wp_die( __('Cheatin&#8217; uh?') );
        }

        check_admin_referer('wp-team-matches-deleteteams');
        ...
        if( $do_action == 'delete' || $do_action2 == 'delete' ) {
            $result = \WP_TeamMatches\Teams::delete_team( $delete );
    ...
    function on_admin_post_sethometeam()
    {
        ...
        check_admin_referer('wp-team-matches-sethometeam');

        $referer = $_REQUEST['_wp_http_referer'];

        extract(Utils::extract_args($_REQUEST, array('id' => array())));

        $error = \WP_TeamMatches\Teams::set_hometeam($id);
    ...
    {
        return $this->team_editor(__('New Team', WP_CLANWARS_TEXTDOMAIN), 'wp-team-matches-addteam', __('Add Team', WP_CLANWARS_TEXTDOMAIN));
    }
    ...
    function on_edit_team()
    {
        ...
        return $this->team_editor(__('Edit Team', WP_CLANWARS_TEXTDOMAIN), 'wp-team-matches-editteam', __('Update Team', WP_CLANWARS_TEXTDOMAIN), $id);
    }
    ...
    function team_editor($page_title, $page_action, $page_submit, $team_id = 0)
    {
        ...
        if($team_id > 0) {
            $teams = \WP_TeamMatches\Teams::get_team( array('id' => $team_id) );
            if(!empty($teams)) {
                $team = reset($teams);
            }
        }
    ...
    function on_load_manage_teams()
    {
        ...
        // ACL checks on edit
        if($act == 'edit') {
            $team = \WP_TeamMatches\Teams::get_team( compact('id') );
            ...
            $this->teams_table = new \WP_TeamMatches\TeamsTable();
    ...
    function handle_add_team() {
        $redirect_url = admin_url( 'admin.php?page=wp-team-matches-teams' );
        ...
        if( \WP_TeamMatches\Teams::add_team( $data ) ) {
    ...
    function handle_edit_team() {
        ...
        if(\WP_TeamMatches\Teams::update_team( $id, $data ) !== false) {
    ...
    function handle_teams_bulk_actions() {
        ...
        if($table_action === 'delete' && isset($_POST['id'])) {
            $result = \WP_TeamMatches\Teams::delete_team( $_POST['id'] );
        ...
        wp_redirect( admin_url( 'admin.php?page=wp-team-matches-teams' ) );
        die();
    }
    ...
    function on_manage_teams()
    {
        ...
        $teams = \WP_TeamMatches\Teams::get_team('id=all&order=asc&orderby=title&limit=' . $limit . '&offset=' . ($limit * ($current_page-1)));
        ...
        $page_links_text = sprintf( '<span class="displaying-num">' . __( 'Displaying %s&#8211;%s of %s' ) . '</span>%s',
                ...
                number_format_i18n( min( $current_page * $limit, 1 ) ),
                '<span class="total-type-count">' . number_format_i18n(1 ) . '</span>',
                $page_links
        );
    ...
    {
        if(!\WP_TeamMatches\ACL::user_can('manage_games')) {
            wp_die( __('Cheatin&#8217; uh?') );
        }

        check_admin_referer('wp-team-matches-gamesop');
        ...
        if(!empty($items)) {
            ...
            switch($action) {
                case 'delete':
                    $result = \WP_TeamMatches\Games::delete_game($items);
    ...
    function export_game($id)
    {
        ...
        $id = (int)$id;
        $games = \WP_TeamMatches\Games::get_game(array('id' => $id));
        $game = current($games);
        ...
        $maplist = \WP_TeamMatches\Maps::get_map(array('game_id' => $game->id));
    ...
    function import_remote_game( $remote_id ) {
        $filename = tempnam( get_temp_dir(), 'wp-team-matches-' );
    ...
    function import_game($zip_file) {
        ...
        // make sure wp-content/uploads/wp-team-matches is not availabe from outside
        $stub_files = array(
        ...
        $game_id = \WP_TeamMatches\Games::add_game($p);
    ...
    {
        return $this->game_editor(__('New Game', WP_CLANWARS_TEXTDOMAIN), 'wp-team-matches-addgame', __('Add Game', WP_CLANWARS_TEXTDOMAIN));
    }
    ...
    function on_edit_game()
    {
        ...
        return $this->game_editor(__('Edit Game', WP_CLANWARS_TEXTDOMAIN), 'wp-team-matches-editgame', __('Update Game', WP_CLANWARS_TEXTDOMAIN), $id);
    }
    ...
    function on_load_manage_games()
    {
        ...
        if( $act === 'add' && !\WP_TeamMatches\ACL::user_can('manage_game', 'all') ) {
            $die = true;
        }
        else if( $act === 'edit' || $act === 'maps' || $act === 'addmap' ) {

            $g = \WP_TeamMatches\Games::get_game(array(
                'id' => ($act === 'maps' || $act === 'addmap' ? $game_id : $id)
            ));

            $die = empty($g) || !\WP_TeamMatches\ACL::user_can('manage_game', $g[0]->id);

        }
        else if( $act === 'editmap' ) {
            $m = \WP_TeamMatches\Maps::get_map( compact('id') );
            $die = empty($m) || !\WP_TeamMatches\ACL::user_can('manage_game', $m[0]->game_id);
        }
        else {
            // setup games table only when displaying all games
            $this->games_table = new \WP_TeamMatches\GamesTable();
        ...
        if( $act === 'maps' ) {
            ...
            $this->maps_table = new \WP_TeamMatches\MapsTable();
    ...
    function handle_add_game() {
        ...
        $upload_id2 = $this->handle_upload( 'g_banner_file' );
        ...
        if( is_wp_error($upload_id2) && $upload_id2->get_error_code() !== self::ErrorUploadNoFile ) {
            Flash::error( $upload_id2->get_error_message() );
            return;
        }
        else if( is_int($upload_id2) ) {
            $data['g_banner_file'] = $upload_id2;
        }

        if( \WP_TeamMatches\Games::add_game( $data ) ) {
            Flash::success( __( 'Added a new game.', WP_CLANWARS_TEXTDOMAIN ) );
            wp_redirect( admin_url( 'admin.php?page=wp-team-matches-games' ) );
            exit();
        }
        else {
    ...
    function handle_edit_game() {
        $id = isset($_GET['id']) ? $_GET['id'] : 0;
        $defaults = array('title' => '', 'abbr' => '', 'delete_image' => false, 'delete_banner' => false);
        $data = Utils::extract_args(stripslashes_deep($_POST), $defaults);
        extract($data);
        ...
        if( is_wp_error($upload_id2) && $upload_id2->get_error_code() !== self::ErrorUploadNoFile ) {
            ...
            $update_data['g_banner_file'] = $upload_id2;
        }
        ...
        if( \WP_TeamMatches\Games::update_game($id, $update_data) !== false ) {
            Flash::success( __( 'Updated a game.', WP_CLANWARS_TEXTDOMAIN ) );
            wp_redirect( admin_url( sprintf('admin.php?page=wp-team-matches-games&act=edit&id=%d', $id) ) );
            exit();
        }
        else {
    ...
    function handle_add_map() {
        ...
        if( \WP_TeamMatches\Maps::add_map($data) !== false ) {
            Flash::success( __( 'Added a map.', WP_CLANWARS_TEXTDOMAIN ) );
            wp_redirect( admin_url( sprintf( 'admin.php?page=wp-team-matches-games&act=maps&game_id=%d', $game_id ) ) );
            exit();
        }
        else {
    ...
    function handle_edit_map() {
        ...
        if( \WP_TeamMatches\Maps::update_map($id, $update_data) !== false ) {
            Flash::success( __( 'Updated a map.', WP_CLANWARS_TEXTDOMAIN ) );
            wp_redirect( admin_url( sprintf( 'admin.php?page=wp-team-matches-games&act=editmap&id=%d', $id ) ) );
            exit();
        } else {
    ...
    function handle_maps_bulk_actions() {
        ...
        if($table_action === 'delete' && isset($_POST['id'])) {
            $result = \WP_TeamMatches\Maps::delete_map( $_POST['id'] );
    ...
    function on_manage_games()
    {
        ...
        $show_add_button = \WP_TeamMatches\ACL::user_can('manage_game', 'all');
    ...
    function game_editor($page_title, $page_action, $page_submit, $game_id = 0)
    {
        ...
        if($game_id > 0) {
            $result = \WP_TeamMatches\Games::get_game(array('id' => $game_id));
            if(!empty($result)) {
                $game = reset($result);
            }
        }
        ...
        $context['attach1'] = isset($game->g_banner_file) ? wp_get_attachment_image($game->g_banner_file, 'thumbnail') : '';
    ...
    function on_admin_post_deletemaps()
    {
        ...
        check_admin_referer('wp-team-matches-deletemaps');
        ...
        if($do_action == 'delete' || $do_action2 == 'delete') {;
            $result = \WP_TeamMatches\Maps::delete_map( $delete );
    ...
    function on_edit_maps()
    {
        ...
        $game = current(\WP_TeamMatches\Games::get_game(array('id' => $game_id)));
    ...
    function on_add_map()
    {
        ...
        $this->map_editor(__('Add Map', WP_CLANWARS_TEXTDOMAIN), 'wp-team-matches-addmap', __('Add Map', WP_CLANWARS_TEXTDOMAIN), $game_id);
    }
    ...
    function on_edit_map()
    {
        ...
        $this->map_editor(__('Edit Map', WP_CLANWARS_TEXTDOMAIN), 'wp-team-matches-editmap', __('Update Map', WP_CLANWARS_TEXTDOMAIN), 0, $id);
    }
    ...
    function map_editor($page_title, $page_action, $page_submit, $game_id, $id = 0)
    {
        ...
        if($id > 0) {
            $t = \WP_TeamMatches\Maps::get_map(array('id' => $id, 'game_id' => $game_id));
    ...
    function on_admin_post_delete_match() {
        if( ! \WP_TeamMatches\ACL::user_can('manage_matches') ) {
            wp_die( __('Cheatin&#8217; uh?') );
        }

        check_admin_referer('wp-team-matches-delete-match');

        if( isset($_REQUEST['id'] ) ) {
            $id = (int) $_REQUEST['id'];
            $result = \WP_TeamMatches\Matches::delete_match( $id );
    ...
    {
        return $this->match_editor(__('Add Match', WP_CLANWARS_TEXTDOMAIN), 'wp-team-matches-matches', __('Add Match', WP_CLANWARS_TEXTDOMAIN));
    }
    ...
    function on_edit_match()
    {
        ...
        return $this->match_editor(__('Edit Match', WP_CLANWARS_TEXTDOMAIN), 'wp-team-matches-matches', __('Update Match', WP_CLANWARS_TEXTDOMAIN), $id);
    }

    function on_ajax_get_maps()
    {
        if(!\WP_TeamMatches\ACL::user_can('manage_games') && !\WP_TeamMatches\ACL::user_can('manage_matches')) {
            wp_die( __('Cheatin&#8217; uh?') );
        }
        ...
        if($game_id > 0) {
            $maps = \WP_TeamMatches\Maps::get_map(array('game_id' => $game_id, 'order' => 'asc', 'orderby' => 'title'));
    ...
    function match_editor($page_title, $page_action, $page_submit, $id = 0)
    {
        ...
        if($id > 0) {
            $matchResult = \WP_TeamMatches\Matches::get_match( array('id' => $id) );
            ...
            if( isset($matchResult) && !empty($matchResult) ) {
                ...
                $rounds = \WP_TeamMatches\Rounds::get_rounds($match->ID);
        ...
        $games = \WP_TeamMatches\Games::get_game(array('id' => \WP_TeamMatches\ACL::user_can('which_games'), 'orderby' => 'title', 'order' => 'asc'));
        $teams = \WP_TeamMatches\Teams::get_team('id=all&orderby=title&order=asc');
        ...
        $team1 = $merged_data['team1'];
        $team2 = $merged_data['team2'];

        $winner_candidates = array($team1=>get_the_title($merged_data['team1']), $team2=>get_the_title($merged_data['team2']));
        $view = new View( 'edit_match' );

        $view->add_helper('html_date_helper', array('\WP_TeamMatches\Utils', 'html_date_helper'));
        $view->add_helper('html_country_select_helper', array('\WP_TeamMatches\Utils', 'html_country_select_helper'));

        $context = compact('page_title', 'page_action', 'page_submit', 'num_comments', 'match_statuses', 'id', 'games', 'teams', 'gallery', 'winner_candidates');
        $context += $merged_data;
    ...
    function quick_pick_team($title, $country) {
        $result = \WP_TeamMatches\Teams::get_team(array('title' => $title, 'limit' => 1));
        if(count($result) === 0) {
            $new_team_id = \WP_TeamMatches\Teams::add_team(array('title' => $title, 'country' => $country));
    ...
    function on_load_manage_matches()
    {
        ...
        if($act === 'add') {
            ...
            $matchResult = \WP_TeamMatches\Matches::get_match(array( 'id' => $id ));
            $current = get_post_meta($matchResult->ID, 'game_id', true);

            if( !\WP_TeamMatches\ACL::user_can('manage_game',$current) ) {
                wp_die( __('Cheatin&#8217; uh?') );
            }

            $media_options['post'] = $matchResult->ID;
        }
        else {
            // setup match table only when displaying all matches
            $this->match_table = new \WP_TeamMatches\MatchTable();
            $this->match_table->prepare_items();
        }

        wp_enqueue_media($media_options);
        wp_enqueue_script('wp-team-matches-matches');
        wp_enqueue_script('wp-team-matches-gallery');
        wp_localize_script('wp-team-matches-matches',
                'wpCWL10n',
                array(
        ...
        if( isset( $_POST['game_id'] ) && !\WP_TeamMatches\ACL::user_can( 'manage_game', $_POST['game_id'] ) ) {
            wp_die( __('Cheatin&#8217; uh?') );
        }
    ...
    function handle_add_match() {
        ...
        $match_id = \WP_TeamMatches\Matches::add_match( $model );

        if(!$match_id) {
            Flash::error(esc_html__( 'Failed to add a match.', WP_CLANWARS_TEXTDOMAIN ) );
            return;
        }
        ...
        foreach($scores as $round_group => $r) {
            ...
            for($i = 0; $i < $num_rounds; $i++) {
                ...
                \WP_TeamMatches\Rounds::add_round( $model );
            }
        }

        \WP_TeamMatches\Matches::update_match_post($match_id);

        Flash::success( esc_html__( 'Added a match.', WP_CLANWARS_TEXTDOMAIN ) );

        wp_redirect( admin_url( 'admin.php?page=wp-team-matches-matches' ) );
        exit();
    }
    ...
    function handle_edit_match() {
        ...
        \WP_TeamMatches\Matches::update_match( $id, $model );


        \WP_TeamMatches\Rounds::delete_rounds_by_match( $id );
        ...
        \WP_TeamMatches\Matches::update_match_post($id);
        Flash::success( esc_html__('Updated a match.', WP_CLANWARS_TEXTDOMAIN) );

        wp_redirect( admin_url( 'admin.php?page=wp-team-matches-matches&act=edit&id=' . $id ) );
        exit();
    }
    ...
    function handle_matches_bulk_actions() {
        ...
        if($table_action === 'delete' && isset($_POST['id'])) {
            $result = \WP_TeamMatches\Matches::delete_match( $_POST['id'] );
    ...
    function on_match_shortcode($match_id) {
        $matchResult = \WP_TeamMatches\Matches::get_match(array('id' => $match_id, 'sum_tickets' => true));
        ...
        $match = $matchResult[0];
        $r = \WP_TeamMatches\Rounds::get_rounds($match->id);
        $rounds = array();
    ...
    function on_browser_shortcode($atts) {
        ...
        $gamesResult = \WP_TeamMatches\Games::get_game('id=all&orderby=title&order=asc');
        ...
        $matches_pagination = \WP_TeamMatches\Matches::get_match($p2,false);
        ...
        $matches = \WP_TeamMatches\Matches::get_match($p);

        $page_links = paginate_links(array(
            'prev_text' => __('&larr;'),
            'next_text' => __('&rarr;'),
            'total' => ceil( count($matches_pagination) / $per_page ),
            'current' => $current_page
        ));

        $page_links_text = sprintf( '<span class="displaying-num">' . __( 'Displaying %s&#8211;%s of %s' ) . '</span>%s',
                number_format_i18n( (($current_page - 1) * $per_page) + 1 ),
                number_format_i18n( min( $current_page * $per_page, count($matches) ) ),
                '<span class="total-type-count">' . number_format_i18n( count($matches) ) . '</span>',
                $page_links
        );

        $output .= '<div class="bmatchlist "><ul class="wp-team-matches-filter">';
        ...
        for($i = 0; $i < sizeof($games); $i++) :
            ...
            $output .= '<li' . ($game->id == $current_game ? ' class="selected"' : '') . '><a class="btn" href="' . $link . '" title="' . esc_attr($game->title) . '">' . esc_html($game->abbr) . '</a></li>';
        endfor;

        $output .= '</ul>';

        $output .= '<ul class="bmatchul block-wrap">';
        ...
        foreach($matches as $index => $match) {
            ...
            if (has_post_thumbnail($team1id)) {
                ...
                $image1 = $img_url; //resize & crop img
            } else {
                $image1 = get_theme_file_uri('assets/img/defaults/defteam70.png');
            }

            if (has_post_thumbnail($team2id)) {
                $thumb = get_post_thumbnail_id($team2id);
                $img_url = wp_get_attachment_url($thumb); //get img URL
                $image2 =$img_url; //resize & crop img
            } else {
                $image2 = get_theme_file_uri('assets/img/defaults/defteam70.png');
            }
            ...
            $output .= '<li class="' . ($index % 2 == 0 ? 'even' : 'odd') . '"><a href="'.esc_url(get_permalink($match->ID)).'">';

            // output game icon

            if (respawn_return_game_banner($match->game_id)) {
                $output .= '<img src="' .respawn_return_game_banner($match->game_id). '" alt="' . esc_attr($match->game_title) . '" class="grayscalebg" /> ';
            }

            $output .= '<div class="bmatchscore">';

                $output .= '<img class="team-logo" src="'.esc_url($image1).'" alt="'.esc_attr($team1_title).'"/>';

                $output .= '<div class="vsresult">';

                    $output .= esc_html($tickets1);;

                    $output .= '<span>VS</span>';

                    $output .= esc_html($tickets2);

                $output .= '</div>';

                $output .= '<img class="team-logo" src="'.esc_url($image2).'" alt="'.esc_attr($team2_title).'"/>';

            $output .= '</div>';

            $output .= '<div class="mbtitle-wrapper">';

                $output .= '<h3>'.date(get_option('date_format'), mysql2date('U', $match->date)) . ' <span>' . date(get_option('time_format'), mysql2date('U', $match->date)) . '</span></h3>';

                $output .= '<h4>';

                    $output .= esc_attr($match->post_title);
        ...
        $output .= '<div class="wp-team-matches-pagination">' .$page_links_text . '</div>';
    ...
    function on_manage_matches()
    {
        ...
        $matches = \WP_TeamMatches\Matches::get_match($condition);
        ...
        foreach ($matches as $match) {
            $game_id = get_post_meta($match->ID, 'game_id', true);
        ...
        $view->add_helper( 'get_country_flag', array('\WP_TeamMatches\Utils', 'get_country_flag') );
    ...
    function on_admin_post_settings() {
        ...
        check_admin_referer('wp-team-matches-settings');
    ...
    function on_admin_post_acl() {
        ...
        check_admin_referer('wp-team-matches-acl');
        ...
        if( isset( $_POST['user'] ) ) {
            ...
            \WP_TeamMatches\ACL::update( $user_id, $data );
        }
    ...
    function on_admin_post_deleteacl() {
        ...
        check_admin_referer('wp-team-matches-deleteacl');
        ...
        if($do_action == 'delete' || $do_action2 == 'delete') {
            ...
            foreach( $users as $key => $user_id ) {
                \WP_TeamMatches\ACL::delete( $user_id );
            }
        }
    ...
    function on_admin_post_import() {
        ...
        check_admin_referer('wp-team-matches-import');
    ...
    function on_admin_post_publish() {
        ...
        check_admin_referer('wp-team-matches-publish');
    ...
    function on_admin_post_logout() {
        check_admin_referer('wp-team-matches-logout');
    ...
    function on_admin_post_update_cloud_account() {
        check_admin_referer('wp-team-matches-update-cloud-account');
    ...
    function on_ajax_game_vote() {
        check_ajax_referer('wp-team-matches-game-vote');
    ...
    function on_settings() {
        ...
        $gamesResult = \WP_TeamMatches\Games::get_game('id=all');
        $acl = \WP_TeamMatches\ACL::get();
        $acl_keys = \WP_TeamMatches\ACL::all_caps();
        ...
        foreach($acl as $user_id => $user_acl) {
            ...
            $allowed_games = \WP_TeamMatches\ACL::user_can('which_games', false, $user_id);
            $user_games = \WP_TeamMatches\Games::get_game(array('id' => $allowed_games, 'orderby' => 'title', 'order' => 'asc'));
    ...
    function on_import_published() {
        ...
        $installed_games = \WP_TeamMatches\Games::get_game('')->getArrayCopy();
        ...
        $this->add_game_browser_script();
        wp_enqueue_script( 'wp-team-matches-login' );

        $view->render( $context );
    }

    function on_import_publish() {
        $publish_action = 'wp-team-matches-publish';
        $active_tab = 'publish';
        $logged_into_cloud = CloudAPI::is_logged_in();
        $cloud_account = CloudAPI::get_user_info();
    ...
    static function on_get_meta($pid) {
    ...
    static function SumTickets($mid) {
        //UPDATED FOR POSTS
        if ($mid > 0) {
    ...
    static function on_mass_update_post_meta($pid, $data) {
    ...
    static function on_delete_match_by_team($id) {
        //CONVERTED TO POSTS
    ...
    function on_players_redirect() {
        header('Location: '.get_admin_url().'edit.php?post_type=player');
    }
...
$wpTeamMatches = new WP_TeamMatches();

register_activation_hook( __FILE__, array(&$wpTeamMatches, 'on_activate'));
