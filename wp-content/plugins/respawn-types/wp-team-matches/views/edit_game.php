...
<div class="wrap wp-team-matches-gameeditor">
    ...
    <form name="team-editor" id="team-editor" method="post" action="<?php echo esc_attr($_SERVER['REQUEST_URI']); ?>" enctype="multipart/form-data">
        ...
        <tr>
            <th scope="row" valign="top"><label for="icon_file"><?php esc_html_e('Banner', 'respawn'); ?></label></th>
            <td>
                <input type="file" name="g_banner_file" id="g_banner_file" />

                <?php if(!empty($attach1)) : ?>
                    <div class="screenshot"><?php echo $attach1; ?></div>
                    <div>
                        <label for="delete_banner"><input type="checkbox" name="delete_banner" id="delete_banner" /> <?php esc_html_e('Delete Banner', 'respawn'); ?></label>
                    </div>
                <?php endif; ?>
            </td>
        </tr>
