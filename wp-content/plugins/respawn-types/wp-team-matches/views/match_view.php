<?php
$current_unixtime = \WP_TeamMatches\Utils::current_time_fixed('timestamp');
...
?>
<div class="wp-team-matches-match-card">
    <div class="wp-team-matches-match-card-page-curl"></div>
    <div class="wp-team-matches-match-card-meta">
...
<?php endif; ?>
    ...
    <div class="wp-team-matches-match-card-header">
        <div class="wp-team-matches-match-card-header-item">
...
<?php else : ?>
            <div class="wp-team-matches-match-card-header-item-no-logo-team-wrap">
                <img src="<?php echo esc_url(WP_CLANWARS_URL . '/images/no-team-logo-light.png'); ?>" width="80" />
                <div class="wp-team-matches-match-card-header-item-team-name"><?php echo esc_html($match->team1_title); ?></div>
            </div>
<?php endif; ?>
        </div>

        <div class="wp-team-matches-match-card-header-item wp-team-matches-match-card-home-team-score"><?php
            echo esc_html( $match->team1_tickets );
        ?></div>

        <div class="wp-team-matches-match-card-header-item wp-team-matches-match-card-status-caption"><?php
        ...
        <div class="wp-team-matches-match-card-header-item wp-team-matches-match-card-visiting-team-score"><?php
...
<?php else : ?>
            <div class="wp-team-matches-match-card-header-item-no-logo-team-wrap">
                <img src="<?php echo esc_url(WP_CLANWARS_URL . '/images/no-team-logo-light.png'); ?>" width="80" />
                <div class="wp-team-matches-match-card-header-item-team-name"><?php echo esc_html($match->team2_title); ?></div>
            </div>
<?php endif; ?>
        </div>
    </div>
    <table class="wp-team-matches-scores-table">
    <thead>
    <tr>
        <th class="wp-team-matches-scores-table-column-heading" style="width: 15%"></th>
<?php
...
?>
        <th class="wp-team-matches-scores-table-column-heading" colspan="2" style="width: <?php echo $col_width; ?>%">
            <div class="wp-team-matches-scores-table-visual">
<?php if (!empty($image_src)) : ?>
                <img src="<?php echo esc_attr($image_src); ?>"
                    alt="<?php echo esc_attr($first->title); ?>"
                    class="wp-team-matches-scores-table-image"
                    width="<?php echo esc_attr($width); ?>" />
<?php endif; ?>
                <div class="wp-team-matches-scores-table-image-caption"><?php echo esc_html($first->title); ?></div>
            </div>
        </th>
<?php endforeach; ?>
    </tr>
...
<?php
    ...
    <tr class="wp-team-matches-scores-table-row <?php echo $current_round % 2 == 0 ? 'wp-team-matches-scores-table-row-even' : '';  ?>">
<?php
    $is_first_column = true;
    foreach($rounds as $map_number => $map_group) :
?>
<?php if ($is_first_column) : $is_first_column = false; ?>
        <td class="wp-team-matches-scores-table-row-heading"><?php
...
<?php if ($current_round < count($map_group)) : ?>
        <td class="wp-team-matches-scores-table-cell"><?php
        ...
        ?></td>
<?php else : ?>
        <td class="wp-team-matches-scores-table-cell"></td>
        <td class="wp-team-matches-scores-table-cell"></td>
<?php endif; ?>
<?php endforeach; ?>
    </tr>
...
</div><!-- .wp-team-matches-match-card -->
...
?>
<p class="wp-team-matches-match-description">
