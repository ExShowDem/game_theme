...
var timer;
var $gps = $('#wp-team-matches-cloud-items');
...
function resetAll() {
    $gps.find('.wp-team-matches-install-button.active')
        .each(function () {
            toggle( $(this), false);
        });
}

$gps.on('click', '.wp-team-matches-install-button:not(:disabled)',
    function (e) {
        var $this = $(this);
...
StarRatingWidget.prototype._vote = function (rating) {
    var settings = window.wpTeamMatchesGameBrowserSettings || {};
    var postData = {
    ...
    this.$itemElement.addClass('wp-team-matches-cloud-item-updating-vote');

    this._xhr = $.post(ajaxurl, postData, this._handleVote.bind(this), 'json');
};

StarRatingWidget.prototype._handleVote = function (response) {
    this.$itemElement.removeClass('wp-team-matches-cloud-item-updating-vote');
...
$gps.find('.wp-team-matches-cloud-item.wp-team-matches-cloud-item-votes-enabled').each(function (i, el) {
